...
#GRAYLIGHT = np.array([[1.0/np.sqrt(3),1.0/np.sqrt(3),1.0/np.sqrt(3)]],dtype='float32')
#GRAYLIGHT = torch.from_numpy(GRAYLIGHT.copy())
GRAYLIGHT = torch.Tensor([[1.0/np.sqrt(3),1.0/np.sqrt(3),1.0/np.sqrt(3)]])
...
def correct_image_linear(img,ill):
    #linear img, linear ill , return linear img
    correct = ill.unsqueeze(2).unsqueeze(3)*torch.sqrt(torch.Tensor([3])).cuda()
    correc_img = torch.div(img,correct+1e-10)
    img_max = torch.max(torch.max(torch.max(correc_img,dim=1)[0],dim=1)[0],dim=1)[0]+1e-10
    img_max = img_max.unsqueeze(1).unsqueeze(1).unsqueeze(1)
    img_normalize = torch.div(correc_img,img_max)
...
def make_image_nolinear(img):
    #linear img,return non-linear img
