...
network = CreateNet_rccnet_2squeeze_fcn_savefeature(model1, model2).cuda()
network.eval()


#visualize and save network conv weights
cnn_net=list(network.squeezenet1_1_A.children())
conv1  =cnn_net[0].weight.cpu().detach().numpy()
#print(conv1.weight.shape) torch.Size([64, 3, 3, 3])
scio.savemat('%s/%s.mat' % ('results','tccnet_weights'), {'conv1':conv1})

# x  =np.arange(network.weight.shape[2]);
# y  =np.arange(network.weight.shape[1]);
# w_fft=torch.rfft(network.weight,2,onesided=False)
# w_fft=torch.unsqueeze(w_fft,0) #(1,H,W,2)
# w_fft=torch.mul(w_fft, network.F_pre.unsqueeze(0).unsqueeze(0))
# w    =torch.irfft(w_fft,2, onesided=False)
# z    =w[0,0,:,:].cpu().detach().numpy();
...
for i in range(1):
    ...
    with torch.no_grad():
        for i,data in enumerate(dataloader_test):
            ...
            savefile='%s/%s/%s' % ('results','tccbenchmark',fn[0].split('/')[-1])
            features=network.get_feature()

            print(savefile, features[0].shape, features[1].shape)
            np.save(savefile, features)
